...
async function getAll(req, res) {;
	...
	const customers = await models.fuel_details.findAndCountAll({
		limit,
  offset
	}
	);
...
async function getById(req, res) {
	...
	const user = await models.fuel_details.findByPk(id);
	if (user) {
		let returnObj = {
			status:'success',
			data : user
...
async function create(req, res) {
	if (req.body.id) {
		...
		// console.log();
	const cust = 	await models.fuel_details.create(req.body);
